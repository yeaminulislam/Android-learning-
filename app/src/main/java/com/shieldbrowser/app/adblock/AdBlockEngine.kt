...
import com.shieldbrowser.app.CrashGuard
...
object AdBlockEngine {
    ...
    private fun loadInternal(context: Context) {
        ...
        try {
            ...
            CrashGuard.record("filters-load", t)
        } finally {
            loading = false
        }
    }
    ...
    private fun abpToRegex(pattern: String): Regex? {
        // Skip pathological patterns: giant regexes are slow and can exhaust
        // the stack on WebView worker threads (StackOverflowError).
        if (pattern.length > 400) return null
        var stars = 0
        for (ch in pattern) if (ch == '*') stars++
     ...
     */
    /**
     * Main match entry. NEVER allowed to throw: this runs on WebView network
     * threads where a rogue regex (StackOverflowError) or any other problem
     * would otherwise kill the whole app mid-browse.
     */
    fun isBlocked(pageUrl: String?, requestUrl: String, isMainFrame: Boolean): Boolean {
        return try {
            isBlockedInternal(pageUrl, requestUrl, isMainFrame)
        } catch (t: Throwable) {
            CrashGuard.record("isBlocked", t)
            false
        }
    }

    private fun isBlockedInternal(pageUrl: String?, requestUrl: String, isMainFrame: Boolean): Boolean {
