...
class BrowserClient(private val tab: BrowserTab, private val host: Host) : WebViewClient() {
    ...
    override fun shouldInterceptRequest(
        ...
        // This runs on a WebView worker thread: any throwable escaping from
        // here can crash the whole app, so block decisions are fully guarded.
        return try {
            blockIfNeeded(view, request)
        } catch (t: Throwable) {
            com.shieldbrowser.app.CrashGuard.record("shouldInterceptRequest", t)
            null
        }
    }
